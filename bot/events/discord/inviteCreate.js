...
export default {
    ...
    async run(client, invite) {
        if (!invite.guild && invite.inviter.id !== client.id) return

        const guild = await client.module.database.Guild(invite.guild.id)

        if (guild.plugins.modlog.events.inviteCreate.enabled) {
            if (!client.channels.cache.get(guild.plugins.modlog.events.inviteCreate.channel)) return client.save(guild.id, { to_change: 'plugins.modlog.events.inviteCreate.enabled', value: false })
            ...
            client.data.modlog_posts[invite.guild.id].push(['inviteCreate', new client.class.modlog({
                color: 'good',
