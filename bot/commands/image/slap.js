...
export default {
    ...
    slash: new Discord.SlashCommandBuilder()
        .addUserOption(option => option.setName('user1').setDescription('User whose profile picture will be slapped.').setRequired(true))
        .addUserOption(option => option.setName('user2').setDescription('User whose profile picture will do the slap.').setRequired(false)),
    ...
    async run(client, message, args, guild, interaction) {
        ...
        if (!member1)
            return new client.class.error('No user specified to slap!', interaction ?? message)
        ...
        return client.send(interaction ? 1 : message.channel, null, null, [
            new Discord.AttachmentBuilder(await new DIG.Batslap().getImage(image1, image2), 'slap.png')
        ])
    }
}